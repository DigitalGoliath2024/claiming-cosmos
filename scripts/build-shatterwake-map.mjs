/**
 * Paints Shatterwake image.png from the color planet-archipelago art.
 * Space + planet lakes → water 106. Land biomes from RGB.
 */
import fs from "fs";
...
const SRC = path.join(root, "map-generator/assets/maps/_shatterwake-color.png");
const OUT_DIR = path.join(root, "map-generator/assets/maps/shatterwake");
const SCALE = 2;
...
const islands = largestIslands(landMask, outW, outH, 800, 12);
console.log({
  source: `${w}x${h}`,
  out: `${outW}x${outH}`,
  counts,
