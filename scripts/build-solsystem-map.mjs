/**
 * Paints Sol System image.png from the panoramic planets-and-sun art.
 *
 * Black space → water 106. Ice giants stay land. Earth's oceans become
 * lakes (blue that touches green continents). The Sun is volcanic land.
...
const SRC = path.join(root, "map-generator/assets/maps/_solsystem-color.png");
const OUT_DIR = path.join(root, "map-generator/assets/maps/solsystem");
const SCALE = 4;
...
/** Dark void, including space seen through crust cracks. */
function isSpaceLike(r, g, b) {
  ...
  if (lum <= 36) return true;
  if (lum <= 58 && chroma < 30) return true;
  return false;
}
...
function landBlue(r, g, b) {
  ...
  // Ice giants (Neptune / Uranus) — land, not lakes.
  if (isBlueWater(r, g, b)) {
    const t = Math.min(1, lum / 180);
...
function floodVoid(rgba, w, h) {
  ...
  // Open anti-aliased crack lips so rifts stay ship-wide after upscale.
  ...
  for (let y = 1; y < h - 1; y++) {
    for (let x = 1; x < w - 1; x++) {
      ...
      const g = rgba[o + 1];
      const b = rgba[o + 2];
      if (isLava(r, g, b) || isGreen(r, g, b) || isBlueWater(r, g, b)) continue;
      const lum = lumOf(r, g, b);
      const chroma = chromaOf(r, g, b);
      if (lum > 72 || chroma > 40) continue;
      if (
        voidMask[i - 1] ||
        voidMask[i + 1] ||
        voidMask[i - w] ||
  ...
  for (let i = 0; i < n; i++) {
    if (extra[i]) voidMask[i] = 1;
  }
  return voidMask;
}

/** Earth's seas: blue that touches green crust. Whole-blue planets stay land. */
function floodPlanetOceans(rgba, w, h, voidMask) {
  ...
  const tryPush = (i) => {
    if (ocean[i] || voidMask[i]) return;
    const o = i * 4;
    if (!isBlueWater(rgba[o], rgba[o + 1], rgba[o + 2])) return;
    ocean[i] = 1;
    q[tail++] = i;
  };
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      if (voidMask[i]) continue;
      const o = i * 4;
      if (!isGreen(rgba[o], rgba[o + 1], rgba[o + 2])) continue;
      if (x > 0) tryPush(i - 1);
      if (x + 1 < w) tryPush(i + 1);
      if (y > 0) tryPush(i - w);
      if (y + 1 < h) tryPush(i + w);
...
const counts = { water: 0, rocky: 0, terra: 0, ice: 0, lava: 0 };
...
const islands = largestIslands(landMask, outW, outH, 200, 16);
console.log({
  source: `${w}x${h}`,
  out: `${outW}x${outH}`,
  counts,
