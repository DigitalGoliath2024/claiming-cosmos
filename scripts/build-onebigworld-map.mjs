/**
 * Paints One Big World image.png from the single circular world art.
 *
 * Black outside the circle → void (water 106). The central blue sea, coasts,
 * rivers, and inland lakes stay lake water — sealed off from the outer void.
 * Land biomes follow the paint. Never emit #000000 walls.
 */
import fs from "fs";
...
const SRC = path.join(root, "map-generator/assets/maps/_onebigworld-color.png");
const OUT_DIR = path.join(root, "map-generator/assets/maps/onebigworld");
const SCALE = 2;
...
function isBlueWater(r, g, b) {
  // Deep navy through bright cyan coasts.
  return b >= 45 && b > r + 15 && b >= g - 20 && r < 110;
}
...
/** All non-void blue is a lake candidate (central sea, rivers, inland pools). */
...
/**
 * Seal the world's seas from outer void so the central ocean packs as lakes,
 * not black space.
 */
...
// At output scale, close any rim leaks so the central sea stays lakes (not void).
{
  const n = outW * outH;
