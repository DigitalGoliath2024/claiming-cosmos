/**
 * Paints Comet's Pass image.png from the shattered-system art.
 * Space + planet oceans → water 106. Land biomes from RGB.
 */
import fs from "fs";
...
const SRC = path.join(root, "map-generator/assets/maps/_cometspass-color.png");
const OUT_DIR = path.join(root, "map-generator/assets/maps/cometspass");
const SCALE = 4;
...
function paintBlue(r, g, b) {
  const lum = (r + g + b) / 3;
  const chroma = Math.max(r, g, b) - Math.min(r, g, b);
  if (lum <= 22) return WATER_BLUE;
  // Dim grey orbit rings stay void, not land.
  if (chroma < 22 && lum <= 72) return WATER_BLUE;

  // Lava / molten cracks
  if (r >= 100 && r >= g + 25 && r >= b + 50 && b < 90) {
    const t = Math.min(1, (r - 100) / 155);
    return 210 + Math.round(t * 40);
  }

  // Lakes and seas on planets (blue, not void-black)
  if (b >= 50 && b > r + 20 && b >= g - 8 && r < 90) {
    return WATER_BLUE;
  }
  ...
  // Tan beaches / pale desert on green worlds — stay terrestrial
  if (
    r >= 90 &&
    g >= 70 &&
    r - b >= 40 &&
...
for (let y = 0; y < outH; y++) {
  for (let x = 0; x < outW; x++) {
    ...
    const si = (sy * w + sx) * 4;
    const di = (y * outW + x) * 4;
    const blue = paintBlue(rgba[si], rgba[si + 1], rgba[si + 2]);
    out[di] = 0;
    out[di + 1] = 0;
    out[di + 2] = blue;
    out[di + 3] = 255;
...
const islands = largestIslands(landMask, outW, outH, 600, 20);
console.log({
  source: `${w}x${h}`,
  out: `${outW}x${outH}`,
  counts,
