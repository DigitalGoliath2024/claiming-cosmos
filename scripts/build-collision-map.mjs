/**
 * Paints Collision image.png from the two-planet impact art.
 *
 * Dark space (including cracks that open to the void) → water 106 so ships
 * can fly from outer space into the planets. Blue lakes stay water 106 so
 * they can sit on the rim of those cracks. Magma stays volcanic land.
 * Never emit #000000 walls — those would block the rifts.
 */
import fs from "fs";
...
const SRC = path.join(root, "map-generator/assets/maps/_collision-color.png");
const OUT_DIR = path.join(root, "map-generator/assets/maps/collision");
const SCALE = 2;
...
for (let y = 0; y < outH; y++) {
  for (let x = 0; x < outW; x++) {
    ...
    const g = rgba[o + 1];
    const b = rgba[o + 2];
    const water = voidMask[si] || isBlueWater(r, g, b);
...
const islands = largestIslands(landMask, outW, outH, 400, 16);
console.log({
  source: `${w}x${h}`,
  out: `${outW}x${outH}`,
  counts,
