...
export class PlayerImpl implements Player {
  ...
  private hasActiveUnitType(unitType: UnitType): boolean {
    for (const unit of this._units) {
      if (unit.type() === unitType && unit.isActive()) {
        return true;
      }
    }
    return false;
  ...
  public buildableUnits(
    ...
    for (let i = 0; i < len; i++) {
      ...
      let existingUnit: Unit | false = false;

      if (tile !== null && this.canBuildUnitType(u, cost) && !inSpawnPhase) {
        if (this.canUpgradeUnitType(u)) {
          existingUnit = this.findExistingUnitToUpgrade(u, tile);
          if (
            existingUnit !== false &&
      ...
      if (canUpgrade !== false) {
        ...
        let steps = MAX_UPGRADE_AMOUNT;
        if (maxLevel !== undefined && existingUnit !== false) {
          steps = Math.max(0, Math.min(steps, maxLevel - existingUnit.level()));
        }
  ...
  private canSpawnUnitType(
    ...
    if (
      this.mg.config().unitInfo(unitType).unique &&
      this.hasActiveUnitType(unitType)
    ) {
      return false;
    }
    switch (unitType) {
