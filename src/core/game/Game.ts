...
export interface UnitInfo {
  ...
  upgradable?: boolean;
  /** Highest level this structure can reach (inclusive). */
  maxLevel?: number;
  /** Only one active building of this type per player. */
  unique?: boolean;
}
...
export const EraDisabledUnits = unitTypeGroup([
  UnitType.AtomBomb,
  UnitType.HydrogenBomb,
  UnitType.MIRV,
  UnitType.MIRVWarhead,
...
export interface UnitParamsMap {
  ...
  [UnitType.InlandBattery]: Record<string, never>;
}
...
export interface Unit {
  ...
  /** True while a structure's hull is below its level-scaled max. */
