...
/**
 * Ports and port guns must be sited from a real coast. Inland sampling lets
...
export class NationStructureBehavior {
  ...
  private maybeUpgradeArmory(): boolean {
    const armory = this.player
      .units(UnitType.Armory)
      .find((unit) => unit.isActive() && !unit.isUnderConstruction());
    if (armory === undefined || !this.player.canUpgradeUnit(armory)) {
      return false;
    }
    this.game.addExecution(
      new UpgradeStructureExecution(this.player, armory.id()),
    );
    return true;
  }
  ...
  private shouldBuildStructure(
    ...
    if (type === UnitType.Armory) {
      return this.player.units(UnitType.Armory).length === 0;
    }
    if (config === undefined) {
      return false;
    }
  ...
  private getSaveUpTarget(): Gold {
    ...
    if (config.isUnitDisabled(UnitType.MissileSilo)) {
      if (config.isUnitDisabled(UnitType.SAMLauncher)) {
        return 0n;
      }
  ...
  private structureSpawnTile(type: UnitType): TileRef | null {
    const tiles = samplesCoastalStructureSites(type)
  ...
  /**
   * Value function for port guns.
   * Prefers coast near an owned port, spaced away from other batteries.
   */
  private portGunValue(): (tile: TileRef) => number {
    ...
    return (tile) => {
      ...
      const otherTiles: Set<TileRef> = new Set(otherGuns.map((u) => u.tile()));
      otherTiles.delete(tile);
      w += nearestTileDist(game, otherTiles, tile);

      // Inland lake shores lose to ocean / real coasts.
      if (game.isOceanShore(tile)) {
        w += 200;
      }

      if (ports.length > 0) {
        const portTiles: Set<TileRef> = new Set(ports.map((u) => u.tile()));
        const nearestPort = nearestTileDist(game, portTiles, tile);
        if (nearestPort !== Infinity) {
