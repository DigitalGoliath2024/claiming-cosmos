...
export class TransportShipExecution implements Execution {
  ...
  private fireLightGun(): void {
    const gunTarget = this.findLightGunTarget();
    if (gunTarget === undefined) {
      return;
    }
    ...
    if (!gunTarget.hasHealth()) {
      this.alreadySentShell.add(gunTarget);
    }
  }

  private findLightGunTarget(): Unit | undefined {
